...
const queueChecker = setInterval(() => {
  ...
  while (queue.length > 0 && currentSpawns.length < maxSpawns) {
    ...
    console.log(
      `next: ${next}`,
      `There are ${currentSpawns.length} current spawns, and ${queue.length} in the queue. The max spawn is ${maxSpawns}`
    );
...
var addToQueue = async function (tokenId, viperLength) {
  ...
  const currentSpawnsIndex = currentSpawns.indexOf(
    formatName(tokenId, viperLength)
  );
  ...
  if (queueIndex < 0 && currentSpawnsIndex < 0) {
    ...
    console.log(
      `${formatName(
        ...
        viperLength,
        false
      )} already in queue at position ${queueIndex}`
    );
    if (queueIndex != 0) {
      console.log(
        `moving ${formatName(tokenId, viperLength, false)} to front of queue`
      );
    ...
    console.log(
      `${formatName(
        ...
        viperLength,
        false
      )} already in currentSpawns at position ${currentSpawnsIndex}`
    );
  }
};
...
var generatePlaceholderAndGif = async function (tokenId, viperLength) {
  const dirPrefix =
    'public/' +
    (process.env.network == 'homestead' ? '' : process.env.network + '-') +
    'gifs/';
  ...
  const filename = path.join(
    __dirname,
    dirPrefix + `${formatName(tokenId, viperLength, false)}/complete.gif`
  );
  ...
  const placeHolderFilename = path.join(
    __dirname,
    `public/viper-loading-loop.gif`
  );
  try {
    ...
    console.log(
      `no placeholder found at ${placeHolderFilename}, begin generation`
    );
  }
...
const pokeOS = async (tokenId, viperLength) => {
  ...
  try {
    ...
    try {
      refreshOpensea(getNetwork(), address, tokenId.toString()).then(
        (response) => {
          console.log(
            `refresh metadata for ${tokenId} on opensea resulted in ${response.status}`,
            { response }
          );
        }
      );
...
const generateGif = async function (tokenId, viperLength) {
  ...
  try {
    fs.accessSync(filename);
    console.log(
      `gif already exists at ${filename}, removing from currentSpans queue`
    );
    currentSpawns.splice(
      currentSpawns.indexOf(`${formatName(tokenId, viperLength, false)}`),
      1
    );
  ...
  const child = spawn(`./node_modules/viper/bin/viper-cli.js`, [
    'generate-gif',
    tokenId,
    viperLength,
    dirPrefix,
  ...
  child.on('error', (error) => {
    console.error(
      `error-${formatName(tokenId, viperLength, false)}: ${error.message}`
    );
  });
  child.on('close', async (code) => {
    ...
    console.log(
      `child process exited with code ${code} while running on ${formatName(
        tokenId,
        viperLength,
        false
      )}`
    ...
    try {
      ...
      numberOfVipers++;
      console.log(
        `gif completed at : ${filename} in a time of ${
          duration / 1000
        } s, average time: ${totalTime / numberOfVipers / 1000} s`
      );
    } catch (e) {
      console.log({ e });
      console.log(
        `exited without completing the gif, adding back to queue: ${formatName(
          tokenId,
          viperLength,
          false
        )}`
      ...
      currentSpawns.splice(
        currentSpawns.indexOf(`${formatName(tokenId, viperLength)}`),
        1
      );
      queue.unshift(`${formatName(tokenId, viperLength)}`);
      return;
    ...
    console.log(
      'done trying to optimize gif, OK to remove from queue whether optimization worked or not'
    );
    currentSpawns.splice(
      currentSpawns.indexOf(`${formatName(tokenId, viperLength)}`),
      1
...
var optimizeGif = async function (filename) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', async (code) => {
      console.log(
        `child process exited with code ${code} while running gifsicle`
      );
      if (code == 0) {
        resolve();
      } else {
...
var generatePlaceholder = async function (tokenId, viperLength) {
  ...
  const filename = `${__dirname}/output/placeholder/${formatName(
    tokenId,
    viperLength,
    false
  )}.png`;
